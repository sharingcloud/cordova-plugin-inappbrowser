...
- (void)setWhitelistedUrl:(CDVInvokedUrlCommand*)command
{
...
- (void)webView:(WKWebView *)theWebView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    bool sameSite = NO;
    if ([url isEqualToString:mainDocumentURL]) {
        sameSite = YES;
    }

    bool isExternal = YES;
    if (self.whitelistedUrl == nil || !sameSite) {
        NSLog(@"[InAppBrowser] URL whitelisting is disabled.");
        isExternal = NO;
    } else {
        NSLog(@"[InAppBrowser] Using whitelisted URL: %@", self.whitelistedUrl);
        NSString *httpPath = [NSString stringWithFormat:@"http://%@", self.whitelistedUrl];
        NSString *httpsPath = [NSString stringWithFormat:@"https://%@", self.whitelistedUrl];

        if ([url hasPrefix:httpPath] || [url hasPrefix:httpsPath]) {
            NSLog(@"[InAppBrowser] Whitelisting url %@ ...", requestString);
            isExternal = NO;
        }
    }

    if (isExternal) {
        NSLog(@"[InAppBrowser] Sending open_external_url event for URL %@", url);
...
@synthesize currentURL, whitelistedUrl;
...
- (void)webView:(WKWebView *)theWebView didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
